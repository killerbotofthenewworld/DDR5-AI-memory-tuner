...
def render_simulation_tab(config: DDR5Configuration) -> None:
    ...
    with col2:
        ...
        # Display metrics
        st.metric("Theoretical Bandwidth", f"{config.bandwidth_gbps:.1f} GB/s")
        st.metric(
            "Effective Bandwidth",
            f"{bandwidth_results['effective_bandwidth_gbps']:.1f} GB/s",
        )
        st.metric("First Word Latency", f"{config.latency_ns:.1f} ns")
        st.metric(
            "Effective Latency",
            f"{latency_results['effective_latency_ns']:.1f} ns",
        )
        st.metric("Power Consumption", f"{power_results['total_power_mw']:.0f} mW")
        st.metric(
            "Power Efficiency",
    ...
    with col_info:
        ...
        if any(jedec_violations.values()):
            ...
            for _, issues in jedec_violations.items():
                for issue in issues:
                    st.write(f"• {issue}")
        else:
